...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBackToApp, showToast, currentUserId }) => {
  ...
  const [editStatus, setEditStatus] = React.useState<'active' | 'trialing' | 'past_due' | 'canceled' | 'none'>('none');
  ...
  const handleUpdate = async () => {
    ...
    setUpdating(true);
    try {
      const expirationIso = editExpires ? new Date(editExpires).toISOString() : '';
      ...
      showToast(`Ο χρήστης ${selectedUser.fullName} ενημερώθηκε επιτυχώς!`, 'success');
  ...
  const trialingUsersCount = users.filter((u) => u.subscriptionStatus === 'trialing').length;
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-slate-900/50 border border-slate-800/60 rounded-[2rem] p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Συνολικοί Χρήστες</p>
            <h4 className="text-2xl font-bold text-slate-50 mt-1">{loading ? '...' : totalUsersCount}</h4>
          </div>
        </div>
        ...
        <div className="bg-slate-900/50 border border-slate-800/60 rounded-[2rem] p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Ενεργές Συνδρομές</p>
            <h4 className="text-2xl font-bold text-slate-50 mt-1">{loading ? '...' : activeSubsCount}</h4>
          </div>
        </div>
        ...
        <div className="bg-slate-900/50 border border-slate-800/60 rounded-[2rem] p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Σε Δοκιμαστική Περίοδο</p>
            <h4 className="text-2xl font-bold text-slate-50 mt-1">{loading ? '...' : trialingUsersCount}</h4>
          </div>
        </div>
        ...
        <div className="bg-slate-900/50 border border-slate-800/60 rounded-[2rem] p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Διαχειριστές (Admins)</p>
            <h4 className="text-2xl font-bold text-slate-50 mt-1">{loading ? '...' : adminsCount}</h4>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 bg-slate-900/30 border border-slate-800/60 rounded-[2.5rem] p-6 md:p-8 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="text-xl font-bold text-slate-50">Διαλογισμός Συνδρομητών</h3>
              <p className="text-slate-400 text-xs">Δείτε ποιοι χρήστες βρίσκονται στην πλατφόρμα και ελέγξτε τις άδειες πρόσβασης.</p>
            </div>
          ...
          {loading ? (
            <div className="py-12 flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Ανάκτηση συνδρομητών...</p>
            </div>
            ...
            <div className="py-12 text-center bg-slate-950/20 border border-dashed border-slate-800/60 rounded-3xl">
              ...
              <p className="text-sm font-bold text-slate-400">Δεν βρέθηκαν χρήστες</p>
              <p className="text-xs text-slate-600 mt-1">Δοκιμάστε άλλον όρο αναζήτησης.</p>
            </div>
          ) : (
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-bold text-slate-50 truncate">{user.fullName || 'Χωρίς Όνομα'}</p>
                          {user.role === 'admin' && (
        ...
        <div className="lg:col-span-5 space-y-6">
          {selectedUser ? (
            ...
            >
              <div>
                <h3 className="text-xl font-bold text-slate-50">Επεξεργασία Χρήστη</h3>
                <p className="text-slate-400 text-xs mt-1">Αλλαγή δικαιωμάτων & συνδρομητικής κατάστασης.</p>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-950/40 border border-slate-850 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-bold text-slate-50 truncate">{selectedUser.fullName}</p>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Ρόλος Συστήματος</label>
                ...
                {/* Subscription Status */}
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Κατάσταση Συνδρομής</label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                    {(['active', 'trialing', 'past_due', 'canceled', 'none'] as const).map((st) => (
                      <button
                        key={st}
                        type="button"
                ...
                {/* Expiry Date */}
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Ημερομηνία Λήξης</label>
                  <div className="relative">
                  ...
                  <p className="text-[9px] text-slate-600 font-medium italic italic ml-1">Αφήστε κενό εάν δεν υπάρχει ημερομηνία λήξης.</p>
                </div>
              </div>
            ...
            <div className="bg-slate-900/20 border border-dashed border-slate-800/60 rounded-[2.5rem] p-8 text-center min-h-[380px] flex flex-col items-center justify-center">
              ...
              <p className="text-sm font-bold text-slate-400">Επιλέξτε έναν Συνδρομητή</p>
              <p className="text-xs text-slate-500 mt-1.5 max-w-xs mx-auto leading-relaxed">
                Κάντε κλικ σε οποιονδήποτε χρήστη για να δείτε τα πλήρη στοιχεία του, να αλλάξετε το ρόλο του ή να τροποποιήσετε χειροκίνητα τη συνδρομή του.
              </p>
            </div>
          )}
        </div>
