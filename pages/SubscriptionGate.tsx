...
import { Sparkles, Check, Flame, Star, ShieldCheck, Heart, LogOut } from 'lucide-react';
...
const SubscriptionGate: React.FC<SubscriptionGateProps> = ({ userEmail, onRefreshSubscription, onLogout }) => {
  ...
  React.useEffect(() => {
    fetch("/api/config-status")
      ...
      .catch((err) => console.error("Error fetching stripe config status on Paywall:", err));
  }, []);
  ...
  const plans = [
    {
      ...
      price: '€19',
      period: 'μήνα',
      description: 'Ιδανικό για νέους wedding filmmakers που ξεκινούν το ταξίδι τους.',
      features: [
    ...
    },
    {
      ...
      badge: 'ΔΗΜΟΦΙΛΕΣ',
      popular: true,
      color: 'from-indigo-600/30 to-purple-600/20 border-indigo-500/50 text-indigo-300 shadow-lg shadow-indigo-500/10',
      btnStyle: 'bg-gradient-to-r from-indigo-500 to-purple-600 hover:opacity-90 text-white shadow-lg shadow-indigo-500/30',
    },
    {
      id: 'lifetime',
      ...
      price: '€299',
      period: 'εφάπαξ',
      description: 'Για καθιερωμένα studios βιντεοσκόπησης που θέλουν μόνιμη πρόσβαση.',
      features: [
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 relative overflow-hidden flex flex-col justify-between p-6">
      ...
      <header className="relative z-10 max-w-7xl mx-auto w-full flex items-center justify-between py-4 border-b border-slate-900">
        <div className="flex items-center gap-2">
          ...
          <span className="font-serif font-bold text-lg text-slate-50">WeddingFilmAI Studio</span>
        </div>
        <button 
          onClick={onLogout}
      ...
      <main className="relative z-10 max-w-5xl mx-auto w-full py-12 flex flex-col items-center gap-10 text-center">
        ...
        <div className="space-y-4 max-w-2xl">
          ...
          <h1 className="text-3xl md:text-5xl font-extrabold text-slate-50 font-serif tracking-tight">
            Απελευθερώστε τη Δύναμη της <span className="text-indigo-400 font-sans">AI</span>
          </h1>
          <p className="text-slate-400 text-sm md:text-base leading-relaxed">
            Η δοκιμαστική σας περίοδος έληξε ή δεν έχετε ενεργή συνδρομή. Επιλέξτε το πακέτο που σας ταιριάζει και αποκτήστε άμεση πρόσβαση στα εργαλεία δημιουργίας σεναρίων και σχεδιασμού ταινιών γάμου.
          </p>
          <p className="text-slate-500 text-xs italic">
            Συνδεδεμένος λογαριασμός: <span className="text-slate-400 font-semibold">{userEmail}</span>
          </p>
        </div>
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <span className={`text-[9px] font-bold tracking-widest px-3 py-1 rounded-full border bg-slate-900/80 ${plan.color}`}>
                    {plan.badge}
                  </span>
                  {plan.popular && (
                ...
                <div className="space-y-1 mb-6">
                  ...
                  <p className="text-slate-400 text-xs h-10">{plan.description}</p>
                </div>

                <div className="flex items-baseline gap-1 mb-8 pb-6 border-b border-slate-900">
      ...
      <footer className="relative z-10 max-w-7xl mx-auto w-full text-center py-6 text-[10px] text-slate-600 uppercase font-bold tracking-widest border-t border-slate-900 flex items-center justify-center gap-1 mt-12">
        Crafted with <Heart size={10} className="text-rose-500 fill-rose-500" /> for creative wedding builders
      </footer>
    </div>
  );
};
