...
import { Calendar, Users, FileText, CheckCircle2, ChevronRight, Clock, Sparkles, AlertTriangle } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ projects, onNavigateProject, onNavigateAI }) => {
    ...
    <div className="space-y-10 animate-in fade-in duration-700 w-full min-w-0">
      ...
      <div className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 p-8 md:p-12 shadow-2xl shadow-indigo-500/20">
        <div className="relative z-10">
          <h2 className="text-3xl md:text-5xl font-bold font-serif mb-3 text-slate-50 leading-tight">Καλώς ήρθατε!</h2>
          <p className="text-indigo-100/80 text-lg max-w-md">Η δημιουργικότητά σας δεν έχει όρια. Δείτε την πρόοδο των έργων σας.</p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <div className="px-4 py-2 rounded-full bg-emerald-500/20 backdrop-blur-md border border-emerald-500/20 text-xs font-bold text-emerald-300 uppercase tracking-widest">
              {projects.filter(p => p.status === 'booked').length} Νέες Κρατήσεις
            </div>
          </div>
        </div>
        ...
        <Sparkles className="absolute right-10 bottom-10 text-slate-50/20" size={120} />
      </div>
      ...
      {!hasApiKey && (
        <div className="bg-amber-500/10 border border-amber-500/20 p-6 rounded-3xl flex items-center gap-4">
          ...
          <div>
            <h4 className="font-bold text-amber-400 uppercase tracking-widest text-[10px]">API Key Required</h4>
            <p className="text-xs text-slate-400">Configure your Gemini API key in settings to unlock AI features.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Calendar} label="Active" value={projects.length.toString()} color="text-indigo-400" bg="bg-indigo-500/5" />
        <StatCard icon={Users} label="Booked" value={projects.filter(p => p.status === 'booked').length.toString()} color="text-sky-400" bg="bg-sky-500/5" />
        <StatCard icon={FileText} label="Editing" value={projects.filter(p => p.status === 'editing').length.toString()} color="text-amber-400" bg="bg-amber-500/5" />
        <StatCard icon={CheckCircle2} label="Done" value={projects.filter(p => p.status === 'delivered').length.toString()} color="text-emerald-400" bg="bg-emerald-500/5" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button className="text-xs text-indigo-400 hover:text-indigo-300 font-bold uppercase tracking-widest transition-colors">View All</button>
          </div>
            ...
            {upcoming.length === 0 ? (
              ...
              >
                <div className="flex items-center gap-5">
                  <div className={`w-16 h-16 rounded-2xl flex flex-col items-center justify-center border-2 font-bold shrink-0 transition-transform group-hover:scale-110 ${getUrgencyColor(project.weddingDate)}`}>
                    <span className="text-[10px] uppercase tracking-tighter opacity-80">
                      {new Date(project.weddingDate).toLocaleString('el-GR', { month: 'short' })}
                    </span>
                  ...
                  <div className="min-w-0">
                    <h4 className="font-bold text-xl text-slate-50 truncate group-hover:text-indigo-400 transition-colors">{project.coupleNames}</h4>
                ...
                <div className="hidden sm:flex items-center gap-4">
                  <span className={`px-4 py-1.5 rounded-full text-[10px] font-bold border uppercase tracking-widest ${STATUS_COLORS[project.status]}`}>
            ...
            {AI_MODULES.slice(0, 6).map(module => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-slate-50 mb-1">{module.title}</p>
                  <p className="text-[10px] text-slate-500 line-clamp-2 leading-relaxed">{module.description}</p>
                </div>
                <ChevronRight size={16} className="hidden lg:block text-slate-600 group-hover:text-slate-50 transition-colors" />
              </button>
            ))}
          </div>
        </div>
...
const StatCard = ({ icon: Icon, label, value, color, bg }: any) => (
  <div className={`glass p-5 md:p-6 rounded-[2rem] border border-slate-800/50 relative overflow-hidden group hover:border-indigo-500/30 transition-all`}>
    ...
    <div className="relative z-10">
      ...
      <p className="text-3xl font-bold text-slate-50 mb-1 tracking-tight">{value}</p>
