...
import { 
  ArrowLeft, Edit2, Trash2, Mail, Phone, ExternalLink, 
  Clock, CheckCircle, Wallet, ListChecks, Sparkles, 
  ChevronRight, Save, X, Calendar, MapPin, FileText,
  FileDown, FileSignature
} from 'lucide-react';
import { motion } from 'framer-motion';
import { STATUS_COLORS, AI_MODULES, STATUS_LABELS } from '../constants';
import { exportProjectToPDF, exportContractToPDF } from '../services/PDFService';
...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ 
  project, userProfile, onUpdateProject, onDeleteProject, onBack, onNavigateAI 
}) => {
  const [isEditing, setIsEditing] = useState(false);
  const [editedProject, setEditedProject] = useState<Project>(project);
  const [activeTab, setActiveTab] = useState<'overview' | 'checklist' | 'ai'>('overview');
  ...
  const toggleDeliverable = (key: keyof Deliverables) => {
    const updated = {
      ...project,
      deliverables: { ...project.deliverables, [key]: !project.deliverables[key] }
    };
  ...
  const completedDeliverables = Object.values(project.deliverables).filter(Boolean).length;
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-right-6 duration-700 pb-24 lg:pb-0 w-full min-w-0">
      ...
      <div className="sticky top-20 lg:top-0 z-40 bg-slate-950/80 backdrop-blur-xl py-4 -mx-6 px-6 lg:mx-0 lg:px-0 flex items-center justify-between border-b border-slate-800/50 lg:border-none">
        <button onClick={onBack} className="flex items-center gap-2 text-slate-400 hover:text-slate-50 transition-colors py-2 group">
        ...
        <div className="flex gap-2">
          ...
          {isEditing ? (
            <div className="flex gap-2">
              <button onClick={handleSave} className="p-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl shadow-lg shadow-emerald-600/20 transition-all">
                <Save size={20} />
              </button>
              <button onClick={() => setIsEditing(false)} className="p-3 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-2xl transition-all">
                <X size={20} />
              </button>
            </div>
          ) : (
            <div className="flex gap-2">
              <button onClick={() => setIsEditing(true)} className="p-3 glass text-indigo-400 hover:bg-indigo-500/10 rounded-2xl transition-all">
                <Edit2 size={20} />
              </button>
              <button onClick={() => onDeleteProject(project.id)} className="p-3 glass text-rose-400 hover:bg-rose-500/10 rounded-2xl transition-all">
                <Trash2 size={20} />
              </button>
            </div>
          )}
      ...
      <section className="relative overflow-hidden rounded-[2.5rem] bg-slate-900/40 border border-slate-800/50 p-8 md:p-12">
        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-4">
            <span className={`px-4 py-1.5 rounded-full text-[10px] font-bold tracking-widest border uppercase ${STATUS_COLORS[project.status]}`}>
            ...
            {isEditing ? (
              <input 
                ...
                onChange={e => setEditedProject({...editedProject, coupleNames: e.target.value})}
              />
            ) : (
              <h2 className="text-4xl md:text-6xl font-bold font-serif text-slate-50 leading-tight">{project.coupleNames}</h2>
            )}
            <div className="flex flex-wrap items-center gap-6 text-slate-400 font-medium">
              <span className="flex items-center gap-2"><Calendar size={20} className="text-indigo-500" /> {new Date(project.weddingDate).toLocaleDateString('el-GR', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
              <span className="flex items-center gap-2"><MapPin size={20} className="text-indigo-500" /> {project.location}</span>
            </div>
          </div>
          
          <div className="bg-indigo-600/10 border border-indigo-500/20 p-6 rounded-[2rem] min-w-[180px] text-center md:text-left">
            <p className="text-[10px] text-indigo-400 font-bold uppercase tracking-widest mb-2">Days to Wedding</p>
            <div className="text-5xl font-bold text-slate-50 flex items-baseline justify-center md:justify-start gap-2">
              {getDaysRemaining()} <span className="text-sm font-normal text-slate-500 uppercase tracking-widest">Days</span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className={`lg:col-span-2 space-y-10 ${activeTab !== 'overview' && 'hidden lg:block'}`}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900/40 border border-slate-800/50 p-8 rounded-[2.5rem] space-y-6">
              ...
              <div className="space-y-4">
                <a href={`mailto:${project.contact.email}`} className="flex items-center justify-between p-5 rounded-2xl bg-slate-950/50 border border-slate-800 hover:border-indigo-500/30 transition-all group">
                  <div className="flex items-center gap-4 min-w-0">
                    <Mail size={18} className="text-slate-500 group-hover:text-indigo-400 transition-colors" />
                    <span className="text-sm font-medium truncate text-slate-300">{project.contact.email}</span>
                  </div>
                  <ExternalLink size={16} className="text-slate-700 group-hover:text-indigo-400" />
                </a>
                <a href={`tel:${project.contact.phone}`} className="flex items-center justify-between p-5 rounded-2xl bg-slate-950/50 border border-slate-800 hover:border-indigo-500/30 transition-all group">
                  <div className="flex items-center gap-4 min-w-0">
                    <Phone size={18} className="text-slate-500 group-hover:text-indigo-400 transition-colors" />
                    <span className="text-sm font-medium truncate text-slate-300">{project.contact.phone}</span>
                  </div>
                  <ExternalLink size={16} className="text-slate-700 group-hover:text-indigo-400" />
                </a>
              </div>
            ...
            <div className="bg-slate-900/40 border border-slate-800/50 p-8 rounded-[2.5rem] space-y-6">
              ...
              <div className="p-6 rounded-2xl bg-slate-950/50 border border-slate-800 space-y-6">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{project.payments.packageName}</span>
                  <span className="text-3xl font-bold text-slate-50">€{project.payments.totalAmount}</span>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    <CheckCircle size={24} className={project.payments.depositPaid ? 'text-emerald-400' : 'text-slate-700'} />
                    <span className="text-[10px] font-bold uppercase tracking-widest">Προκαταβολή</span>
                  </button>
                  <button 
                  ...
                  >
                    <CheckCircle size={24} className={project.payments.balancePaid ? 'text-emerald-400' : 'text-slate-700'} />
                    <span className="text-[10px] font-bold uppercase tracking-widest">Εξόφληση</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className={`space-y-8 ${activeTab !== 'checklist' && 'hidden lg:block'}`}>
          <section className="bg-slate-900/40 border border-slate-800/50 p-8 rounded-[2.5rem] space-y-8">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">{completedDeliverables}/{totalDeliverables}</span>
            </div>
            ...
            <div className="space-y-3">
              {(Object.keys(project.deliverables) as (keyof Deliverables)[]).map((deliverableKey) => (
                <button 
                ...
                >
                  <span className="text-sm font-bold uppercase tracking-widest">{deliverableLabels[deliverableKey] || String(deliverableKey)}</span>
          ...
          <section className="bg-slate-900/40 border border-slate-800/50 p-8 rounded-[2.5rem] space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              {AI_MODULES.slice(0, 8).map(module => (
                ...
                >
                  <span className="text-4xl group-hover:scale-110 transition-transform">{module.icon}</span>
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500 group-hover:text-slate-50">{module.title}</span>
                </button>
              ))}
            </div>
          </section>
