...
const SidebarItem = ({ icon: Icon, label, tab, activeTab, setActiveTab, setSelectedProjectId, setSelectedModuleId, setIsSidebarOpen }: any) => (
  <button
    onClick={() => {
      setActiveTab(tab);
    ...
    className={`w-full flex items-center gap-3 px-4 py-4 rounded-xl transition-all duration-200 ${
      activeTab === tab ? 'sidebar-active text-indigo-400 bg-indigo-500/10' : 'text-slate-400 hover:text-slate-50 hover:bg-slate-800/50'
    }`}
  >
    <Icon size={22} />
...
const App: React.FC = () => {
  const [activeTab, setActiveTab] = React.useState<'dashboard' | 'projects' | 'ai-studio' | 'settings' | 'admin'>('dashboard');
  const [selectedProjectId, setSelectedProjectId] = React.useState<string | null>(null);
  const [selectedModuleId, setSelectedModuleId] = React.useState<AIModuleType | null>(null);
  ...
  // Monitor Authentication and Fetch personal data
  React.useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      setAuthChecking(true);
      if (user) {
        try {
          // ensure user has a profile doc in Firestore (creates default if needed)
    ...
    if (sessionId && verifiedRef.current !== sessionId) {
      ...
      .then(async (data) => {
        if (data && data.success && data.userId === firebaseUser.uid) {
          ...
          showToast(`Συγχαρητήρια! Η συνδρομή σας ενεργοποιήθηκε επιτυχώς${data.isSimulated ? ' (Simulated)' : ''}!`);
        } else {
  ...
  const handleUpdateProject = async (updated: Project) => {
    ...
    try {
      ...
      setProjects((prev) => prev.map(pro => pro.id === updated.id ? updated : pro));
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center gap-4 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <p className="text-sm font-semibold tracking-widest text-slate-400 mt-6 uppercase animate-pulse">Φόρτωση Studio...</p>
        </div>
      </div>
    );
  }
  ...
  if (!firebaseUser) {
    return (
      <React.Fragment>
        <Auth 
          onSignInSuccess={(user) => setFirebaseUser(user)}
      ...
      {globalLoading && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center">
          <div className="flex flex-col items-center">
            ...
            <p className="text-xs font-semibold text-indigo-400 mt-4 uppercase tracking-widest">Επεξεργασία...</p>
          </div>
        </div>
      )}
      ...
      <aside className="hidden lg:flex flex-col w-72 border-r border-slate-800/60 bg-slate-950/50 backdrop-blur-xl sticky top-0 h-screen">
        <div className="p-8">
          <div className="flex items-center gap-3 mb-10">
            ...
            <h1 className="text-xl font-bold font-serif gradient-text tracking-tight">WeddingFilmAI</h1>
          </div>

          <nav className="space-y-1.5">
            <SidebarItem icon={LayoutDashboard} label="Dashboard" tab="dashboard" activeTab={activeTab} setActiveTab={setActiveTab} setSelectedProjectId={setSelectedProjectId} setSelectedModuleId={setSelectedModuleId} setIsSidebarOpen={setIsSidebarOpen} />
            <SidebarItem icon={FolderKanban} label="Projects" tab="projects" activeTab={activeTab} setActiveTab={setActiveTab} setSelectedProjectId={setSelectedProjectId} setSelectedModuleId={setSelectedModuleId} setIsSidebarOpen={setIsSidebarOpen} />
            <SidebarItem icon={Sparkles} label="AI Studio" tab="ai-studio" activeTab={activeTab} setActiveTab={setActiveTab} setSelectedProjectId={setSelectedProjectId} setSelectedModuleId={setSelectedModuleId} setIsSidebarOpen={setIsSidebarOpen} />
            <SidebarItem icon={SettingsIcon} label="Settings" tab="settings" activeTab={activeTab} setActiveTab={setActiveTab} setSelectedProjectId={setSelectedProjectId} setSelectedModuleId={setSelectedModuleId} setIsSidebarOpen={setIsSidebarOpen} />
            {userProfile.role === 'admin' && (
              <SidebarItem icon={Shield} label="Admin Portal" tab="admin" activeTab={activeTab} setActiveTab={setActiveTab} setSelectedProjectId={setSelectedProjectId} setSelectedModuleId={setSelectedModuleId} setIsSidebarOpen={setIsSidebarOpen} />
            )}
          </nav>
        </div>
        ...
        <div className="mt-auto p-6 border-t border-slate-800/50">
          <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-900/50 border border-slate-800/50">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-slate-50 truncate">{userProfile.fullName || 'User'}</p>
              <p className="text-[10px] text-slate-500 uppercase tracking-wider truncate">{userProfile.businessName || 'My Studio'}</p>
            </div>
            <button onClick={handleLogout} className="text-slate-500 hover:text-rose-400 transition-colors p-1.5" title="Αποσύνδεση">
              <LogOut size={18} />
            </button>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 min-h-screen pb-24 lg:pb-0">
        ...
        <header className="lg:hidden sticky top-0 z-40 bg-slate-950/80 backdrop-blur-md border-b border-slate-800/50 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-serif font-bold text-lg gradient-text">WeddingFilmAI</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleLogout} className="text-slate-500 hover:text-rose-400 transition-colors p-1" title="Αποσύνδεση">
              <LogOut size={18} />
            </button>
          </div>
        </header>
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-slate-950/90 backdrop-blur-lg border-t border-slate-800/50 px-6 py-3 pb-8 z-50 flex items-center justify-between">
          <NavButton icon={LayoutDashboard} label="Home" active={activeTab === 'dashboard'} onClick={() => { setActiveTab('dashboard'); setSelectedProjectId(null); }} />
          <NavButton icon={FolderKanban} label="Projects" active={activeTab === 'projects'} onClick={() => { setActiveTab('projects'); setSelectedProjectId(null); }} />
          <div className="relative -top-6">
            <button 
              onClick={() => { setActiveTab('ai-studio'); setSelectedModuleId(null); }}
            ...
            >
              <Sparkles size={24} fill={activeTab === 'ai-studio' ? "currentColor" : "none"} />
            </button>
          </div>
          <NavButton icon={UserIcon} label="Profile" active={activeTab === 'settings'} onClick={() => { setActiveTab('settings'); setSelectedProjectId(null); }} />
          {userProfile.role === 'admin' && (
            <NavButton icon={Shield} label="Admin" active={activeTab === 'admin'} onClick={() => { setActiveTab('admin'); setSelectedProjectId(null); }} />
          )}
          <button 
...
const NavButton = ({ icon: Icon, label, active, onClick, className = "" }: any) => (
  <button 
    onClick={onClick}
